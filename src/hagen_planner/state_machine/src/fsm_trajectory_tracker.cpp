...
void FSM_Trajectory_Tracker::init(ros::NodeHandle& nh)
{
  ...
  rebound_optimizer.reset(new ego_planner::BsplineOptimizer);
  ...
  // projector_ = laser_geometry::LaserProjection();

  /* ---------- callback ---------- */
  // exec_timer_ = node_.createTimer(ros::Duration(0.01), &FSM_Trajectory_Tracker::execFSMCallback, this);
  // cmd_timer_ = node_.createTimer(ros::Duration(1.0/sampling_rate), &FSM_Trajectory_Tracker::cmdCallback, this);
  ...
  outfile_waypoints.open(waypoints_file, std::ios_base::app);
  outfile_bspline.open(bspline_file, std::ios_base::app);
}
...
void FSM_Trajectory_Tracker::waypointCallback(const nav_msgs::PathConstPtr& msg)
{
  ...
  outfile_waypoints<< 0 <<","<< 1 << "," <<stop_pose[0] << "," << stop_pose[1] << ","<< stop_pose[2];
  std::cout<< "=================file writing"<< std::endl;
  stop_yaw_angle =  getYawFromQuat(odom.pose.pose.orientation);
  trigger_ = true;
  ...
  for(int i=0; i<(int)msg->poses.size(); i++){
    ...
    outfile_waypoints<< "," << wp[0] << "," << wp[1] << ","<< wp[2];
...
void FSM_Trajectory_Tracker::solverThread() {
  while(is_allowed_for_execution){
    ...
    trajectroy_tracker->u0(3,0) = odom.twist.twist.angular.z;
...
int main(int argc, char** argv)
{
  ...
  ros::NodeHandle node; // I think this line is not used later
  ros::NodeHandle nh("~");
